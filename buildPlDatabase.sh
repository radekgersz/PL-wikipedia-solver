...
# By default, the latest Polish Wikipedia dump will be downloaded.
# If a download date in the format YYYYMMDD is provided as the first argument, it will be used instead.
if [[ $# -eq 0 ]]; then
  DOWNLOAD_DATE=$(wget -q -O- https://dumps.wikimedia.your.org/plwiki/ | grep -Po '\d{8}' | sort | tail -n1)
else
  if [ ${#1} -ne 8 ]; then
...
DOWNLOAD_URL="https://dumps.wikimedia.your.org/plwiki/$DOWNLOAD_DATE"
TORRENT_URL="https://tools.wmflabs.org/dump-torrents/plwiki/$DOWNLOAD_DATE"
...
############################
if [ ! -f sdow-pl.sqlite ]; then
  echo
  echo "[INFO] Creating redirects table"
  time pigz -dc redirects.with_ids.txt.gz | sqlite3 sdow-pl.sqlite ".read $ROOT_DIR/../sql/createRedirectsTable.sql"

  echo
  echo "[INFO] Creating pages table"
  time pigz -dc pages.pruned.txt.gz | sqlite3 sdow-pl.sqlite ".read $ROOT_DIR/../sql/createPagesTable.sql"

  echo
  echo "[INFO] Creating links table"
  time pigz -dc links.with_counts.txt.gz | sqlite3 sdow-pl.sqlite ".read $ROOT_DIR/../sql/createLinksTable.sql"

  echo
  echo "[INFO] Compressing SQLite file"
  time pigz --best --keep sdow-pl.sqlite
else
